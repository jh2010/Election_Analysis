...
with open(file_to_save, "w") as txt_file:
    ...
    for county_name in county_votes:        
        ...
        election_results_by_county = (
            f"{county_name}: {county_vote_percentage:.1f}% ({county_votes_count:,})\n")
        print(election_results_by_county)
         # 6e: Save the county votes to a text file.
        txt_file.write(election_results_by_county)
         # 6f: Write a decision statement to determine the winning county and get its vote count.
        if (county_votes_count > county_largest_turnout_votes):
            county_largest_turnout_votes = county_votes_count
